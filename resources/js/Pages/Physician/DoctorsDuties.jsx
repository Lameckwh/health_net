import { WEEK_DUTIES } from "@/const/data";
import React, { useState } from "react";
import { FaPlusCircle } from "react-icons/fa";

function DoctorsDuties() {
  ...
  const handleSubmit = (e) => {
    ...
    // Clear the form
    setNewDuty({ doctorName: "", day: "", shift: "" });
  };

  return (
    <div className="p-4 rounded-lg bg-[#EEEEEE] w-full">
      <div className="flex justify-between mb-4">
        <h2 className="text-sm font-bold text-blue-500">Doctor's Weekly Duties</h2>
        <button onClick={() => setIsModalOpen(true)}>
          <FaPlusCircle className="text-blue-500" />
        </button>
      </div>
      <ul className="space-y-1">
        {WEEK_DUTIES.map((duty) => (
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="bg-white rounded-lg p-6 w-96">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                <label className="block text-gray-700 mb-1" htmlFor="shift">
                  Shift (e.g., Morning, Afternoon, Night)
                </label>
                <input
                  type="text"
                  name="shift"
